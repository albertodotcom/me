...
it("renders NavItems inside DomDelayedUpdate", () => {
  ...
  const wrapper = shallow(<Nav {...defaultProps} navItems={navItems} />);
...
it("calls the onNavItemClick with the new menu Index", () => {
  ...
  const wrapper = shallow(<Nav {...defaultProps} onNavItemClick={aSpyFunc} />);
...
it("shows only the selectedNavItem", () => {
  ...
  const wrapper = shallow(
    <Nav {...defaultProps} navItems={navItems} selectedItem={selectedItem} />
  );

  expect(wrapper.find(NavItem).length).toEqual(3);
  expect(wrapper.find(NavItem).at(selectedItem).props().icon).toEqual(
    thisTestIcon
  );
  expect(wrapper.find(NavItem).at(selectedItem).props().text).toEqual(
    thisTestText
  );
  expect(
    wrapper.find(NavItem).at(selectedItem).props().animationClasses.hideRoot
  ).toEqual("");

  expect(wrapper.find(NavItem).at(0).props().animationClasses.hideRoot).toEqual(
    "hideRoot"
  );
  expect(wrapper.find(NavItem).at(1).props().animationClasses.hideRoot).toEqual(
    "hideRoot"
  );
});
...
describe("navigation buttons", () => {
  it("calls onNavItemClick with the next index", () => {
    ...
    const wrapper = shallow(
      <Nav {...defaultProps} onNavItemClick={aSpyFunc} selectedItem={0} />
    );
  ...
  it("calls onNavItemClick with the 0 if currentIndex is the last one", () => {
    ...
    const wrapper = shallow(
      <Nav {...defaultProps} onNavItemClick={aSpyFunc} selectedItem={1} />
    );
...
describe("hamburger menu", () => {
  it("doesnt show the hamburger menu if there are no active sections", () => {
    const wrapper = shallow(<Nav {...defaultProps} selectedItem={-1} />);
  ...
  it("shows the hamburger menu if there is an active section", () => {
    const wrapper = shallow(<Nav {...defaultProps} selectedItem={0} />);
